...
17. For major visual or shell rewrites, include at least one check that explicitly evaluates hero composition, brand dominance, graph-first exploration feel, unmistakable moving shader/ascii background behavior across the full site, and whether the enhanced state is materially stronger than the static fallback. Also cite the screenshot paths or public source references you reviewed.
18. If the feature includes deployment or release work, validate the production-like output after build and use `gh` or `npx vercel` only when the feature explicitly requires it. Do not resume release/deployment work until the `graph-first-shell-and-token-policy` milestone has passed validation.
19. Stop every process you started. In your handoff, include the exact browser checks, viewport/fallback checks, the cited screenshot/source evidence, and any remaining environment blocker.
